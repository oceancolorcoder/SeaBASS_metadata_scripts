...
inFile1 = fullfile(basePath,'field_data','metadata',campaign,'Arctic22_Alakanuk_NortonSound_SBA_Radiometry_Field_Log.xlsx');
outPath = fullfile(basePath,'field_data','metadata',campaign);

for cruises={'Alakanuk','NortonSound'}
    cruise = char(cruises);
    metadata = struct();
    %% Combined spreadsheet frome Blake (Alakanuk) and Scott (Norton Sound)
    [anc_data, anc_txt, anc_raw] = xlsread(inFile1, cruise);    

    if strcmpi(cruise,'Alakanuk')
        % Alakanuk has more than one raw file per station
        % Keep all here to average the SBA files
        anc_txt(1:6,:) = []; % header

        for i=1:size(anc_data,1)
            metadata(i).filename = anc_txt(i,2);
            ...
            metadata(i).cloud = anc_data(i,11); % percent

            wave_str = anc_txt{i,10};
            metadata(i).waves = mean(str2double(strsplit(wave_str,'-'))); % average the range [m]
